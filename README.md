...
**Estado de implementación:** Supertonic está cableado en el worker igual que su predecesor: (1) sintetiza la **referencia** de la frase del usuario para el score de pronunciación; (2) **reproduce** el follow-up del tutor. Sin medir todavía, y declarado como pendiente: la validación perceptual de la voz en el navegador de este proyecto y la latencia de síntesis sobre WASM (las cifras que publica el autor son de ejecución nativa CPU/GPU, no de WASM en navegador).
