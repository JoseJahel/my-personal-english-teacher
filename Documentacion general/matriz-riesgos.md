...
| R6 | Score 0–100 mide locutor, no pronunciación | Baja (política aplicada) | Alta | Pregunta de tribunal: “¿no están midiendo el timbre?” | Medido (#95, re-medido #58): Δlocutor **11.3** ≳ Δerror vocal **9.9** (ratio **1.14**) en `scorePronunciationFromMonoPcm`. Conversación **sin** 0–100; la nota está en **Repetir**. Fuente: `speaker-bias-invariants.ts`, `calibracion-score-pronunciacion.md` §6 | Jahel (#95/#58) |
...
| R8 | Cadena user ≠ ref TTS sesga el score | Baja (cadena unificada) | Media | Score “raro” entre conversación y drill | Issue #73 **hecho**: `prepareSpeechPcmForModels` (resample + pasa-banda 80 Hz–7.5 kHz) se aplica a user **y** a la ref TTS antes del score. Una pasada causal, no `filtfilt`. MFCC sigue 25/10/13/40 | Jahel (#73) |
