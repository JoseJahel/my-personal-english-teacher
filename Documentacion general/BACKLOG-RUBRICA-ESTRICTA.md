...
**Estado 2026-08-17:** el lote de rúbrica (#57–#79, #81, #92–#98) está **consumido**
en GitHub. **#80 se cierra** con este sync.
...
~~`#65` hecho~~ → ~~`#73` hecho~~ → ~~`#66` hecho~~ → ~~`#67` hecho~~ → ~~`#58` hecho~~ → ~~`#76` hecho~~ → ~~`#74` hecho~~ → ~~`#71` hecho~~

### Oleada 3 — Remate Final
~~`#61` hecho~~ → ~~`#77` hecho~~ → ~~`#79` hecho~~ → ~~`#62` hecho~~ → ~~`#63` hecho~~ → ~~`#72` hecho~~

### Oleada 4 — Laboratorio y defensa (Jahel, #92–#98)
~~`#97`~~ → ~~`#96`~~ → ~~`#93`~~ → ~~`#92`~~ → ~~`#95`~~ → ~~`#94`~~ → ~~`#98`~~ (todas hechas)
...
| 2 — Laboratorio | — | lote cerrado |
| 3 — Remate | — | lote cerrado (PRs #116, #117, #118) |
| 4 — Laboratorio y defensa | — | lote cerrado |
...
|---|--------|--------|---------------------------|
| 57 | Sync matriz/README | Hecho | Honestidad del repo |
...
| 62 | PDF documento técnico | Hecho | Artefacto de entrega del curso |
| 63 | Filtrado ruido | Hecho (PR #117) | RF-23 / Wiener STFT solo en ASR usuario |
...
|---|----------------------|--------|-----------|
| 65 | Medir/endurecer anti-alias del **resample a 16 kHz** | Hecho | Extensión de decisión Avance 1 de captura nativa |
...
| 69 | Diff visual de gramática | Hecho | Feedback con colores del enunciado |
| 70 | **Ensayo de UI** sin mic ni descarga | Hecho | `#practice-mock` + `pnpm build:ensayo` |
| 71 | Bitácora + evidencias | Hecho | Decisión→métrica→test de *este* repo |
| 72 | Hábitos desde historial IndexedDB | Hecho (PR #118) | Racha local + umbral good 72 |
| 73 | Pasa-banda + misma cadena user/ref TTS | Hecho | Evita sesgo de ruta en *nuestro* score |
| 74 | Métricas de borde VAD (ms) | Hecho | Auto-stop que ya cableamos |
...
| 77 | Normalizar números/siglas **antes del TTS** | Hecho | Escenarios restaurante/aeropuerto + SpeechT5 |
| 78 | Explicación **en español** de la corrección T5 | Hecho | Decisión de producto “UI en español” |
| 79 | Tarjeta de señales del turno en el chat | Hecho (PR #116) | Métricas por valor; sin PCM en el mensaje |
...
|---|--------|--------|---------------------------|
| 92 | Remuestreo FIR **44.1 y 48** + polifase | Hecho | #65 es el FIR mínimo; este cubre 44.1 racional y coste |
| 93 | STFT/YIN live sobre **PCM real** | Hecho | #59 pinta; este usa *nuestra* FFT/YIN, no el Analyser |
| 94 | Auditoría de **encadenado** MFCC | Hecho | #67 son dorados; este caza el bug de escala entre etapas |
| 95 | Sesgo de locutor vs error, política de score | Hecho | #29 calibra umbral; este mide Δ y decide conversación/drill |
| 96 | Chat ASR+T5 **sin** esperar tutor | Hecho | #61 es el perfil; este define el presupuesto 2 s como feedback |
| 97 | Kit de defensa **local** | Hecho | Riesgos + Q&A con *nuestras* cifras; sin host cloud |
| 98 | Contratos + mocks inyectables | Hecho | #70 deja de ser maniquí: mismo `HomeScreen` |
...
13. ~~**#74** VAD ms~~ (hecho)  
14. ~~**#71** bitácora~~ (hecho)

### Oleada remate

15. ~~**#61** latencia~~ (hecho)  
16. ~~**#77** TTS números~~ (hecho)  
17. ~~**#79** tarjeta de turno~~ (hecho, PR #116)  
18. ~~**#62** PDF~~ (hecho)  
19. ~~**#63** / **#72** ruido y hábitos~~ (hecho, PRs #117 / #118)

### Oleada laboratorio y defensa (en paralelo a A2 solo #97/#96)

20. ~~**#97** kit defensa~~ (hecho)  
21. ~~**#96** feedback progresivo~~ (hecho)  
22. ~~**#93** PCM live~~ (hecho)  
23. ~~**#92** FIR multi-tasa~~ (hecho)  
24. ~~**#95** sesgo locutor~~ (hecho)  
25. ~~**#94** encadenado MFCC~~ (hecho)  
26. ~~**#98** mocks~~ (hecho)  
