 ...
 */
import { computeRootMeanSquareEnergy, sanitizeNonFiniteSamples } from '../dsp/signal-energy'
 ...
 *
 * Non-finite samples (NaN/Infinity, from a corrupted decode or driver
 * glitch) are sanitized to silence first, so they cannot corrupt the frame
 * energy scan or leak downstream toward Whisper.
 */
export function trimSpeechSilence(
  ...
  const sanitizedSamples = sanitizeNonFiniteSamples(samples)
  ...
  const frameCount = Math.floor(sanitizedSamples.length / frameSampleCount)
  if (frameCount < 2) {
    return sanitizedSamples.slice()
  }
  const frameRms: number[] = []
  let maxFrameRms = 0
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    const start = frameIndex * frameSampleCount
    const frame = sanitizedSamples.subarray(start, start + frameSampleCount)
  ...
  const endSampleExclusive =
    endFrame >= frameCount - 1
      ? sanitizedSamples.length
      : Math.min(sanitizedSamples.length, (endFrame + 1) * frameSampleCount)
  if (startSample <= 0 && endSampleExclusive >= sanitizedSamples.length) {
    return sanitizedSamples.slice()
  }
  return sanitizedSamples.slice(startSample, endSampleExclusive)
}
