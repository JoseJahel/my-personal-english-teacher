 ...
 *
 * Non-finite samples (NaN/Infinity) are ignored when measuring the peak, so
 * a single corrupted sample cannot inflate the measured peak. When scaling
 * is applied, any corrupted sample is written out as silence (0) instead of
 * an amplified NaN/Infinity.
 */
export function normalizePeakAmplitude(
  ...
  for (let sampleIndex = 0; sampleIndex < samples.length; sampleIndex += 1) {
    const sampleValue = samples[sampleIndex]!
    normalizedSamples[sampleIndex] = Number.isFinite(sampleValue) ? sampleValue * gain : 0
  }
